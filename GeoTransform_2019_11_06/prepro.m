...
if model_id < 80
    ...
    rpc     = evalin('base','rpc');
    n_image = evalin('base','n_image');
    for j = 1 : n_image
        for i = 1 : 5
            points (: , i + 6 , j) = (points(: , i + 1 , j) - rpc(i , j)) / rpc(i + 5 , j);
        end
    end
    ...
    for j = 1 : n_image
        for i = 1 : length(points(: , 1 , 1))
            points(i , 12 : 13 , j) = rc(points(i , 9 : 11 , j), rpc (: , j));
        end
        for i = 1 : 2
            stdb (: , i , j) = sqrt(((points(: , i + 6 , j) - points(: , i + 11, j))' * (points(: , i + 6 , j) - points(: , i + 11 , j))) / length(points(: , 1 , 1))) * rpc(i + 6 , j);
        fprintf(fid, 'Standard devition in bias for image (%1d): %5.2f pixel\n\n', sqrt(stdb(: , : , i) * stdb(: , : , i)'));
        % ===== Compensation models =====
        [points(:, 14 : 15 , i), fid] = bias_compensation(points(: , : , i), rpc(: , i), fid);
        end
    end
end
%% ===== Loading of GCPs to be ignored =====
if meta(9) == 1
    display('Attention! This part must be arranged regarding 3D RFM!')
    pause
    Sng = input('GCP ID to be ignored. exp. [427 526 258]: ');
    for i = 1 : length(Sng)
...
if Sc > 0
    for i = 1 : n_image
        [gcp(: , : , i), icp(: , : , i)] = generate_gcp_icp(points(: , : , i), Sc);
    end
